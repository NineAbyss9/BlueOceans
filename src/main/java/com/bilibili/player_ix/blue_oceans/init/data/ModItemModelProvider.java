...
extends ItemModelProvider
{
    ...
    protected void registerModels()
    {
        BoDataGenHelper.ITEM_REGISTRIES.forEach(object -> {
            ...
            {
                block(object.getId().getPath(), object.getId().getPath());
            } else if (item instanceof Food food)
            {
                String loc = food.getLoc();
                String address = food.getAddress();
                basic(loc.isEmpty() ? object.getId().getPath() : food.getLoc(), address.isEmpty() ? "food/" + item :
                        address);
    ...
    public ItemModelBuilder block(String loc, String blockPath)
    {
        return getBuilder(modid + ":item/block/" + loc)
                .parent(new ModelFile.UncheckedModelFile(modLoc("block/" + blockPath)));
    }

    public ItemModelBuilder blockWithTex(String loc, String texPath)
    {
        return getBuilder(modid + ":item/block/" + loc)
                .parent(new ModelFile.UncheckedModelFile("item/generated"))
                .texture("layer0", modLoc("item/block/" + texPath));
    }
    ...
    {
        return getBuilder(ForgeRegistries.ITEMS.getKey(item).toString())
