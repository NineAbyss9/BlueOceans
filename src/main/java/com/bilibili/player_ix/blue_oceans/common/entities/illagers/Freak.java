...
implements IFlagMob, IAnimatedMob {
    ...
    public Freak(EntityType<? extends Freak> p_21683_, Level p_21684_) {
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(1, new CrazyTargetGoal<>(this));
    ...
    public boolean isAlliedTo(@Nullable Entity p_32665_) {
        if (p_32665_ == null) {
            ...
            return true;
        } else if (p_32665_ instanceof OwnableMob heartOfHorror) {
    ...
    public boolean doHurtTarget(Entity p_21372_) {
        if (this.isCrazy()) {
            if (p_21372_.isAlive() && p_21372_ instanceof LivingEntity lie) {
                ...
                lie.hurt(this.damageSources().mobAttack(this), 1.5F +
        ...
        if (this.isAlive()) {
            this.setHealth(this.getHealth() + 1.9f + (this.isCrazy() ? 2f : 0.5f));
        }
    ...
    static {
        DATA_SECOND_PHASE = SynchedEntityData.defineId(Freak.class, EntityDataSerializers.BOOLEAN);
        DATA_FLAGS = SynchedEntityData.defineId(Freak.class, EntityDataSerializers.INT);
        DATA_ANIM_TICK = SynchedEntityData.defineId(Freak.class, EntityDataSerializers.INT);
    }
