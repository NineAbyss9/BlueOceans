...
implements IXUtilUser {
    ...
    public AttackBehavior(PathfinderMob pMob, double pSpeed, boolean pCanPenalize) {
        this.mob = pMob;
        this.speedModifier = pSpeed;
        this.canPenalize = pCanPenalize;
            ...
            if ((this.followingTargetEvenIfNotSeen || this.mob.getSensing().hasLineOfSight(livingentity))
                ...
                if (this.canPenalize) {
                    ...
                    if (this.mob.getNavigation().getPath() != null) {
                        Node node = this.mob.getNavigation().getPath().getEndNode();
                        if (node != null && livingentity.distanceToSqr(node.asVec3()) < 1.0) {
                ...
                this.ticksUntilNextPathRecalculation = this.adjustedTickDelay(ticksUntilNextPathRecalculation);
            }
    ...
    protected void checkAndPerformAttack(LivingEntity pTarget, double pDistance) {
        ...
        if (pDistance <= d0 && this.ticksUntilNextAttack <= 0) {
    ...
    protected double getAttackReachSqr(LivingEntity pTarget) {
        return this.mob.getBbWidth() * 2.0F * this.mob.getBbWidth() * 2.0F + pTarget.getBbWidth();
    }
